...
public class SetManager {
    ...
    public SXAttributeData getSetAttributes(Player player) {
        ...
        for (Map.Entry<String, Integer> entry : setPieceCounts.entrySet()) {
            ...
            if (setData != null) {
                ...
                // 更新玩家装备上的套装状态显示
                updateSetLoreForPlayer(player, setData, pieceCount);
            }
        }
     ...
     * @param player 目标玩家
     * @param setData 套装数据
     * @param pieceCount 当前套装件数
     */
    private void updateSetLoreForPlayer(Player player, SetData setData, int pieceCount) {
        // 获取玩家装备栏的所有物品
        ItemStack[] armorContents = player.getInventory().getArmorContents();
        // 遍历装备栏的每个位置
            ...
            if (item != null && item.hasItemMeta()) {
                ...
                if (meta.hasLore()) {
                    ...
                    for (int j = 0; j < lore.size(); j++) {
                        ...
                        if (line.replace("§", "&").contains(setData.getName())) {
                            // 更新套装状态显示（例如：3/4）
                            lore.set(j, line.substring(0, line.lastIndexOf("(")) + "(" + pieceCount + "/4)");
                            updated = true;
                            break;
                        }
                    }
                    // 如果lore有更新，应用更改
                    if (updated) {
                        meta.setLore(lore);  // 设置新的lore
                        item.setItemMeta(meta);  // 更新物品元数据
                        armorContents[i] = item;  // 更新装备栏中的物品
                    }
                }
            }
        }
